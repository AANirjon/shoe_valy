...
const MyOrder = (props) => {
    ...
    useEffect(() => {
        fetch('https://ahnaf-shoe.herokuapp.com/orders')
            .then(res => res.json())
    ...
    const handleCancelOrder = id => {
        ...
        swal({
            title: "Are you sure?",
            text: "If you deleted, you will not be able to recover your order!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
            .then((willDelete) => {
                if (willDelete) {
                    const url = `https://ahnaf-shoe.herokuapp.com/orders/${id}`;
                    fetch(url, {
                        method: 'DELETE'
                    })
                        .then(res => res.json())
